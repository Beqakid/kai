...
export default function DashboardPage() {
  ...
  useEffect(() => {
    async function load() {
      try {
        const [dashRes, actRes, momRes, recRes, dispRes, revRes] =
          await Promise.all([
            fetch("/api/dashboard")
            ...
            fetch("/api/activity?limit=8")
              .then((r) => r.json() as any)
              .catch(() => ({})),
            fetch("/api/kai/momentum")
              .then((r) => r.json() as any)
              .catch(() => ({})),
            fetch("/api/kai/recommendations?status=proposed")
              .then((r) => r.json() as any)
              .catch(() => ({})),
            fetch("/api/dispatch")
              .then((r) => r.json() as any)
        ...
        setRecentDispatches(
          Array.isArray(dispRes?.dispatches)
            ? dispRes.dispatches.slice(0, 5)
            : []
        );
    ...
    <div className="space-y-6">
      ...
      <div className="grid md:grid-cols-2 gap-4">
        ...
        {momentum.length > 0 && (
          <div className="bg-white rounded-xl border border-gray-100 p-4">
            <h3 className="font-semibold text-gray-900 mb-3 flex items-center gap-2">
              <TrendingUp className="w-4 h-4 text-green-600" /> Project Momentum
            </h3>
            <div className="space-y-2">
              {momentum.slice(0, 6).map((m: any) => (
                <div key={m.projectId} className="flex items-center justify-between text-sm">
                  <Link href={`/projects/${m.slug}`} className="text-gray-700 hover:text-indigo-600 truncate flex-1">
                    {m.projectName}
                  </Link>
        ...
        {staleProjects.length > 0 && (
          <div className="bg-white rounded-xl border border-gray-100 p-4">
            <h3 className="font-semibold text-gray-900 mb-3 flex items-center gap-2">
              <AlertCircle className="w-4 h-4 text-orange-500" /> Stale Projects
            </h3>
            <div className="space-y-2">
              {staleProjects.slice(0, 5).map((s: any) => (
                <div key={s.project.id} className="text-sm">
                  <Link href={`/projects/${s.project.slug}`} className="text-gray-700 hover:text-indigo-600 font-medium">
                    {s.project.name}
                  </Link>
        ...
        {patterns.length > 0 && (
          <div className="bg-white rounded-xl border border-gray-100 p-4">
            <h3 className="font-semibold text-gray-900 mb-3 flex items-center gap-2">
              <AlertTriangle className="w-4 h-4 text-red-500" /> Repeated Blocker Themes
            </h3>
            <div className="space-y-2">
          ...
          <div className="bg-white rounded-xl border border-gray-100 p-4 md:col-span-2">
            <div className="flex items-center justify-between mb-3">
              <h3 className="font-semibold text-gray-900 flex items-center gap-2">
                <Lightbulb className="w-4 h-4 text-amber-500" /> Kai Recommendations
              </h3>
              <Link href="/kai/recommendations" className="text-xs text-indigo-600 hover:text-indigo-800 flex items-center gap-1">
              ...
              {recs.slice(0, 4).map((r: any) => (
                <div key={r.id} className="flex items-start gap-3 text-sm">
                  ...
                  <div className="min-w-0">
                    <p className="text-gray-700 font-medium">{r.title}</p>
                    <p className="text-xs text-gray-400 truncate">{r.recommendation}</p>
                  </div>
                </div>
              ))}
            </div>
      ...
      {/* Kai Task Orchestrator — full Phase 6 panel */}
      ...
      {revenueStats &&
        (revenueStats.totalOpportunities > 0 || revenueStats.proposalNeeded > 0) && (
          <div className="grid md:grid-cols-2 gap-4">
            <div className="bg-gradient-to-r from-green-50 to-emerald-50 rounded-xl border border-green-200 p-4">
              <div className="flex items-center justify-between mb-3">
                <h3 className="font-semibold text-green-900 flex items-center gap-2">
                  <DollarSign className="w-4 h-4" /> Closest to Revenue
                </h3>
                <Link href="/revenue" className="text-xs text-green-600 hover:text-green-800 flex items-center gap-1">
                  View All <ArrowRight className="w-3 h-3" />
                </Link>
              </div>
              {revenueStats.closestToMoney?.length > 0 ? (
                <div className="space-y-2">
                  {revenueStats.closestToMoney.slice(0, 3).map((o: any) => (
                    <div key={o.id} className="flex items-center justify-between text-sm">
                      <div className="min-w-0">
                        <p className="text-gray-700 font-medium truncate">{o.title}</p>
                        <p className="text-xs text-gray-500">{o.project_name || "No project"}</p>
                      </div>
                      <div className="text-right flex-shrink-0 ml-2">
                        <p className="font-bold text-green-700">
                          {"$"}{(o.estimated_value || 0).toLocaleString()}
                        </p>
                        <p className="text-xs text-gray-400">{o.probability}%</p>
                      </div>
                    </div>
                  ))}
                </div>
            ...
            <div className="bg-white rounded-xl border border-gray-100 p-4">
              <h3 className="font-semibold text-gray-900 mb-3 flex items-center gap-2">
                <Target className="w-4 h-4 text-amber-500" /> Revenue Summary
              </h3>
              <div className="grid grid-cols-2 gap-2 text-sm">
                <div>
                  <span className="text-gray-400">Opportunities</span>
                  <p className="font-bold text-gray-900">{revenueStats.totalOpportunities}</p>
                </div>
                <div>
                  <span className="text-gray-400">Potential Value</span>
                  <p className="font-bold text-green-700">
                    {"$"}{(revenueStats.totalPotentialValue || 0).toLocaleString()}
                  </p>
                </div>
                <div>
                  <span className="text-gray-400">Proposals Needed</span>
                  <p className="font-bold text-amber-600">{revenueStats.proposalNeeded}</p>
                </div>
                <div>
                  <span className="text-gray-400">Won / Lost</span>
                  <p className="font-bold text-gray-900">
                    {revenueStats.wonCount} / {revenueStats.lostCount}
                  </p>
                </div>
              </div>
            </div>
      ...
      {recentDispatches.length > 0 && (
        <div className="bg-white rounded-xl border border-gray-100 p-4">
          <div className="flex items-center justify-between mb-3">
            <h3 className="font-semibold text-gray-900 flex items-center gap-2">
              <Send className="w-4 h-4 text-blue-500" /> Recent Dispatches
            </h3>
            <Link href="/dispatch" className="text-xs text-indigo-600 hover:text-indigo-800 flex items-center gap-1">
              View All <ArrowRight className="w-3 h-3" />
            </Link>
          </div>
          <div className="space-y-2">
            {recentDispatches.map((d: any) => (
              <Link
                key={d.id}
                href={`/dispatch/${d.id}`}
                className="flex items-center justify-between text-sm hover:bg-gray-50 rounded-lg p-2 -mx-1"
              >
                <div className="min-w-0">
                  <p className="text-gray-700 font-medium truncate">{d.title}</p>
                  <p className="text-xs text-gray-400">{d.project_name || "No project"}</p>
                </div>
                <span
      ...
      <div className="grid md:grid-cols-3 gap-4">
        <div className="bg-white rounded-xl border border-gray-100 p-4">
          <h3 className="font-semibold text-gray-900 mb-3 flex items-center gap-2">
            <AlertTriangle className="w-4 h-4 text-red-500" /> Active Blockers
          </h3>
            ...
            <div className="space-y-2">
              {activeBlockers.map((b: any) => (
                <div key={b.id} className="text-sm">
                  <p className="text-gray-700 font-medium truncate">{b.title}</p>
                  <p className="text-xs text-gray-400">{b.project_name}</p>
                </div>
              ))}
            </div>
          )}
        ...
        <div className="bg-white rounded-xl border border-gray-100 p-4">
          <h3 className="font-semibold text-gray-900 mb-3 flex items-center gap-2">
            <Scale className="w-4 h-4 text-purple-500" /> Pending Decisions
          </h3>
            ...
            <div className="space-y-2">
              {pendingDecisions.map((d: any) => (
                <div key={d.id} className="text-sm">
                  <p className="text-gray-700 font-medium truncate">{d.title}</p>
                  <p className="text-xs text-gray-400">{d.project_name}</p>
                </div>
              ))}
            </div>
          )}
